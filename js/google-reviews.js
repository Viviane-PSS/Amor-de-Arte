  ...
  service.findPlaceFromQuery(
    ...
    function (results, status) {
      ...
      if (badge && found.rating != null) {
        badge.innerHTML = buildBadgeHTML(
          found.rating,
          found.user_ratings_total,
        );
      }
...
function buildBadgeHTML(rating, total) {
  ...
  for (var i = 1; i <= 5; i++) {
    stars +=
      '<span class="gstar" style="color:' +
      (i <= Math.round(rating) ? "#FBBC05" : "#ccc") +
      '">★</span>';
  }
  return (
...
function buildCardHTML(review) {
  ...
  var avatar = review.profile_photo_url
    ? '<img src="' +
      ...
      '" alt="' +
      review.author_name +
      '" class="g-reviewer-img">'
    : '<div class="g-reviewer-initial" style="background-color:' +
      avatarColor(review.author_name) +
      '">' +
